...
def main():
    ...
    if random.random() > 0.99:
        transforms = T.Compose([T.CenterCrop(48),])
    else:
        transforms = None
    trn_ds = dataloader.MRI_Dataset(TRAIN_IN, TRAIN_OUT,transforms,NORM )
    trn_dl = DataLoader(trn_ds, TRAIN_BATCH_SIZE, shuffle = True, num_workers = WORKERS)
    val_ds = dataloader.MRI_Dataset(VAL_IN, VAL_OUT,transforms,NORM )
